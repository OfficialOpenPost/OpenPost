...
> **Package manager:** npm (no yarn/pnpm in repo). **CLI:** `openpost-cli` v0.2.3 only (bin `openpost-cli`, no `create-openpost` alias).
...
├── cli/                     # openpost-cli v0.2.3 → dist/index.js (single bin openpost-cli)
...
## 9) CLI (`openpost-cli` v0.2.3, single bin)
...
cd cli && npm run build && node dist/index.js --help  # → openpost-cli v0.2.3
