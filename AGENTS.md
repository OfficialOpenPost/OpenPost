...
> **Package manager:** npm (no yarn/pnpm in repo). **CLI:** `openpost-cli` v0.2.2 only (bin `openpost-cli`, no `create-openpost` alias).
...
├── cli/                     # openpost-cli v0.2.2 → dist/index.js (single bin openpost-cli)
...
## 9) CLI (`openpost-cli` v0.2.2, single bin)
...
cd cli && npm run build && node dist/index.js --help  # → openpost-cli v0.2.2
