...
async function exchangeCode(cmsUrl: string, code: string) {
  ...
  while (attempts < 3) {
    ...
    try {
      ...
      return data as { token: string; projectId: string; projectSlug?: string; projectName?: string; tokenPrefix?: string };
    } catch (e: any) {
...
async function runInit(args: Record<string, any>) {
  ...
  let exchange: { token: string; projectId: string; projectName?: string; projectSlug?: string } | null = null;
  try {
...
# Optional: override project slug for multi-project setups
# OPENPOST_PROJECT_SLUG=${projectSlug || ""}
