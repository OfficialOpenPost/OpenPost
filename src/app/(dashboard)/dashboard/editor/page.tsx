        ...
        <div className="flex flex-1 items-center justify-between px-4 min-w-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0 hidden sm:block">
              <p className="text-[10px] font-bold uppercase tracking-widest text-text-tertiary leading-none">Post</p>
              <p className="text-sm font-semibold text-navy truncate leading-none mt-1 max-w-[200px] md:max-w-[280px]">{title || "Untitled Post"}</p>
            </div>
            <span className="hidden md:inline-flex"><StatusIndicator status={saveStatus} /></span>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <span className="hidden xl:inline text-xs text-text-tertiary whitespace-nowrap">{words} words · {minutes} min</span>
            <button onClick={() => setPreview(true)} className="hidden sm:inline-flex items-center gap-1.5 rounded-full border border-border bg-white px-4 py-2 text-sm font-medium hover:bg-surface-raised whitespace-nowrap">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full bg-[#FEA611] px-5 py-2 text-sm font-bold text-[#2D3440] shadow-sm hover:bg-[#FE990E] transition whitespace-nowrap"
            >
              <Sparkles className="h-4 w-4" /> {status === "published" ? "Update" : status === "scheduled" ? "Scheduled" : "Publish"}
            </button>
            <button
              onClick={() => setShowSidebar(!showSidebar)}
              className="flex h-8 w-8 items-center justify-center rounded-full border bg-white hover:bg-surface-raised lg:hidden shrink-0"
            ...
            <button
              onClick={() => setShowSidebar(!showSidebar)}
              className="hidden lg:flex h-8 w-8 items-center justify-center rounded-full border bg-white hover:bg-surface-raised shrink-0"
              aria-label="Toggle sidebar"
            >
